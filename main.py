...
async def fetch_user_id(username, update=None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=HEADERS) as resp:
            if resp.status == 200:
                ...
                logging.warning(f"fetch_user_id failed: {resp.status} — {reason}")
                if update:
                    await update.message.reply_text(
                        f"❌ Ошибка {resp.status} при получении ID @{username}.\nПричина: {reason}"
                    )
    return None


async def fetch_tweets(user_id, update=None):
    url = (
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=HEADERS) as resp:
            if resp.status == 200:
                ...
                reason = await resp.text()
                logging.warning(f"fetch_tweets failed: {resp.status} — {reason}")
                if update:
                    await update.message.reply_text(
                        f"❌ Ошибка {resp.status} при получении твитов.\nПричина: {reason}"
                    )
    return []
...
async def track_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if username not in tracked_users[chat_id]:
        ...
        user_id = await fetch_user_id(username, update)
        if user_id:
            tweets = await fetch_tweets(user_id, update)
